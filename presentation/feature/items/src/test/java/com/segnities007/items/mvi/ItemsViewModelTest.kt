...
class ItemsViewModelTest {
    ...
    @Test
    fun `Given BottomSheet is displayed When user taps close button Then BottomSheet is hidden`() =
        runTest(testDispatcher) {
    ...
    @Test
    fun `Given image path is not set When user captures photo Then image path is set`() =
        runTest(testDispatcher) {
    ...
    @Test
    fun `Given user is on any screen When navigating to ItemsList Then currentRoute becomes ItemsList`() =
        runTest(testDispatcher) {
            ...
            val state = viewModel.uiState.value
            assertEquals(com.segnities007.navigation.NavKeys.ItemsList, state.data.currentRoute)
        }

    @Test
    fun `Given user is on ItemsList screen When tapping camera button Then navigates to camera screen`() =
        runTest(testDispatcher) {
            ...
            val state = viewModel.uiState.value
            assertEquals(com.segnities007.navigation.NavKeys.CameraCapture, state.data.currentRoute)
        }
}
