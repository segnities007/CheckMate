...
class IcsTemplateRepositoryImpl(
    ...
    private fun parseIcsLine(line: String, eventData: MutableMap<String, String>) {
    ...
    private fun parseRecurrenceRule(rrule: String): com.segnities007.model.calendar.RecurrenceRule? {
        ...
        return try {
            ...
            com.segnities007.model.calendar.RecurrenceRule(frequency = frequency)
        } catch (e: Exception) {
            null
        }
    }
    ...
    private fun groupEventsByDayAndTime(events: List<CalendarEvent>): List<EventGroup> {
        return events.groupBy { event ->
            val dayOfWeek = when (event.startDateTime.dayOfWeek) {
    ...
    private fun generateTemplateTitle(group: EventGroup): String {
        val dayName = when (group.dayOfWeek) {
        ...
        val timeName = when (group.timeSlot) {
            TimeSlot.MORNING -> "朝"
    ...
    private suspend fun generateItemIdsForEvents(events: List<CalendarEvent>): List<Int> {
        ...
        return events.flatMap { event ->
            try {
                ...
                Log.d("IcsTemplate", "Gemini AI推奨: Event=${event.title}, Items=${aiRecommendedIds.map { id -> 
